<table class="table solsoTable table-hover {{!empty($refresh) ? 'solsoRefresh' : '' }}" data-all="{{sizeof($pmttrnsmast_obj)}}">
	...
	<tbody>
		@foreach($pmttrnsmast_obj as $crt => $v)
		<tr>
			...
			<td class="col-xs-1 col-sm-1 col-md-1">
				...
				data-toggle="modal" data-target="#solsoCrudModal"   rel="editpmtgrpmast" data-edittransactionid="{{$v->transaction_id}}"
				data-href="{{URL::to('editpmttrnsmastform/'.$v->transaction_id.'/')}}" data-modal-title="Edit Entity">
				<i class="fa fa-pencil"></i>Edit</button>


			</td>
			<td class="col-xs-1 col-sm-1 col-md-1">
				<form class="formDelete" method="POST"  action="pmttrnsmast-del" enctype="multipart/form-data">
					<input id= "_token" type="hidden" name="_token" value="{{ csrf_token() }}" />
					<input id= "deleteID" type="hidden" name="deleteID" value="{{$v->transaction_id}}" />
